...
### 4.35
思路同4.18题,高度为h的最小结点AVL树，其左右子树分别为 高度为h-1 和 h-2的最小结点AVL树。然后递归即可。  
见```MyAVL.h```

### 4.36 (重要)
生成理想平衡二叉树的两个方法:  
1. 上一题中左右子树分别为 高度为h-1 和 h-2    
2. 4.34题中```rndInt(int low,int upper)```不生成随机数而是```return (low+upper)/2```即可。
...
### 4.40(重要)
二叉树的层序遍历.主要借助队列,见```MyTree.h```
在LeetCode 102中有类似的题目。

### 4.41 4.42(重要)
B树的实现,在后面章节进行实现。现略。

### 4.43(重要)
此处要注意的一点是,兄弟结点必须和自己有共同的父亲。
![](https://i.imgur.com/18GKqpe.png)
