...
a. 比如左式堆:
			 ...
			 4 5  6 7
			8   
建堆的时候进行：1 2 3 4 5 6 7 8    
第一轮下来： 1/2 3/4 5/6 7/8    
第二轮下来： （1/2/3/4） （5/6/7/8）    
第三轮下来： 即为最终结果    
可以看到，第一次进行合并的时候需要花费2*1*(N/2),这里2代表比较和交换子树需要的常数时间，1代表由于只有1个根节点。(N/2)即代表这么多个组合。       
第二次进行合并需要花费2*2*(N/4).  第2个2即由于右路径有2个需要比较交换的结点。      
...
略  
### 6.28  
将键1到2^k-1依序插入初始为空的斜堆，可以形成一棵理想平衡树。**成立**    
证明如4.20 6.21  

### 6.29（重要）（斜堆的buildHeap)
可以，使用和6.25相同的算法。   
练习6.25中的所有单个运算估计都是平摊的，而不是最坏情况，但是根据平摊分析的定义，这些估计的和是序列的最坏情况的界。
